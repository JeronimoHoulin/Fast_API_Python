...
@cliente.get('/clientes', tags=["Clientes"],
             response_model=list[SchemaCliente])
def get_Clientes():
...
def create_Cliente(cliente: SchemaCliente):
    ...
    nuevo_cliente["contrasena"] = funcion_cifrar.encrypt(
        cliente.contrasena.encode("utf-8"))
    ...
    return connection.execute(
        clientes.select().where(clientes.c.id == respuesta.lastrowid)).first()
...
def edit_Cliente(id: int, cliente: SchemaCliente):
    ...
    update_cliente["contrasena"] = funcion_cifrar.encrypt(
        cliente.contrasena.encode("utf-8"))

    connection.execute(
        clientes.update().values(update_cliente).where(clientes.c.id == id))
